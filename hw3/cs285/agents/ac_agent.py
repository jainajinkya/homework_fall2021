...
class ACAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        advantage = self.estimate_advantage(
            ob_no=ob_no, next_ob_no=next_ob_no, re_n=re_n, terminal_n=terminal_n
        )

        for _ in range(self.agent_params["num_actor_updates_per_agent_update"]):
            actor_loss = self.actor.update(
                observations=ob_no, actions=ac_na, adv_n=advantage
            )
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        adv_n = re_n + self.gamma * V_s_next * (terminal_n == False) - V_s
