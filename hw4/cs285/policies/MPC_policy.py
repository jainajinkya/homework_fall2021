...
class MPCPolicy(BasePolicy):
        ...
        if self.sample_strategy == "random" or (
            ...
            for i in range(self.cem_iterations):
                ...
                if not i:
                    ...
                    action_sequences = np.random.normal(loc=elite_mean, scale=elite_std, size=(num_sequences, horizon, self.ac_dim))
                ...
                elites = action_sequences[np.argsort(rews)[-self.cem_num_elites:]]
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        rews = np.empty([len(self.dyn_models), len(candidate_action_sequences)])
        for i, model in enumerate(self.dyn_models):
            rews[i, :] = self.calculate_sum_of_rewards(
